function [avg_cvar_cop,avg_cvar_sp,avg_cvar_saa,cop10,cop90,sp10,sp90,saa10,saa90,avg_T_cop,avg_T_sp,avg_T_saa] = outperformance_CV_general(Npoints,K)
...
for i = 1:trial
    ...
    epsilon_p_cop = epsilon_p_cop_value(data_cons, data_in{i,1},K,epsilon_cop)
    epsilon_p_sp = epsilon_p_sp_value(data_cons, data_in{i,1},K,epsilon_sp)
    
    % copositive programming    
    if cop_cont
        [x_cop,obj_cop,runtime_cop] = Newsvendor_PLD_cop_general(data_cons,data_in{i,1},epsilon_cop,epsilon_p_cop,K);
    ...
    if sp_cont
        [x_sp,obj_sp,runtime_sp] = Newsvendor_PLD_sp_general(data_cons,data_in{i,1},epsilon_sp,epsilon_p_sp,K);
        if runtime_sp > 30*60
            sp_cont = false;
            T_sp(i) = NaN;
            cvar_sp_list(i) = NaN;

...
% K fold CV
% [epsilon0] = LS_epsilon0_value(data);
% epsilon0 = 0;
...
% values = 0;
% for k = 1:K
%     value{k} = epsilon{k}*norm([eye(a);zeros(N+1,a)]*Y{k}-lambda{k}*[zeros(a+N,1);1]*[zeros(a+N,1);1]',1) + trace([eye(a);zeros(N+1,a)]*Y{k}*Omega{k});
%     values = values + value{k};
% end
%      
% obj = values; % Frobenius-norm + last component
...
%     value{k} = epsilon{k}*norm(A{k} + B{k},1) + trace((A{k} + B{k})*Omega{k});
    values = values + value{k};
end

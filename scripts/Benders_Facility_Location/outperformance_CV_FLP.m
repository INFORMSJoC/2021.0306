...
% end 
% for i = 1:trial
%     data0_in{i,1} = FLP_generate_data(20);
% end

disp("finish generating data")

% if Npoints > 20
%     epsilon_benders_list = zeros(trial,1);
%     i = 0;
%     while i < trial
%         i = i+1;
%         epsilon_benders_list(i) = FLP_epsilon_benders_value(data0_in{i,1},f,C);
%     end
%     epsilon_benders_fix = mode(epsilon_benders_list);
% end
...
for i = 1:trial
    % Cross Validation
    if Npoints <= 20
%         epsilon_benders = FLP_epsilon_benders_value(data_in{i,1},f,C)
        epsilon_benders = 1;
    else
%         epsilon_benders = epsilon_benders_fix
        epsilon_benders = 1;
    end
...
%     gap_saa(i) = (cvar_saa_list(i) - exp_benders_list(i))/cvar_saa_list(i);
%     gap_sp(i) = (cvar_sp_list(i) - exp_benders_list(i))/cvar_sp_list(i);

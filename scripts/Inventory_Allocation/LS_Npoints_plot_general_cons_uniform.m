...
% fig2 = figure(2);
% hold on
% plot_dro_T = plot(Npoints(1:num),avg_T_cop_list(1:num),'linewidth', 3,'color', color(1,:)); 
% plot_wass_T=plot(Npoints(1:num),avg_T_wass_list(1:num),'linewidth', 3,'color', color(2,:)); 
% 
% grid on
% set(gca, 'FontSize', font_size - 6);
% xlabel('Number of In-sample Points', 'Interpreter', 'latex', 'FontSize', font_size);
% ylabel('Average Runtime(sec)', 'Interpreter', 'latex', 'FontSize', font_size);
% lgd = legend([plot_dro_T plot_wass_T], 'PLDR','Wasserstein', 'Location', 'northeast');
% set(lgd,'Interpreter','latex', 'FontSize', font_size-6);
% % saveas(gcf,'fig2-all','svg')